...
static void IRAM_ATTR tca6416a_int_isr_handler(void){
  xQueueSendFromISR(tca6416a_int_queue,NULL,NULL);
}

static void tca6416a_int_task(void){
  static uint32_t pvBuf = 0;
  while(true){
    if(xQueueReceive(tca6416a_int_queue,&pvBuf,portMAX_DELAY)){
...
void ext_io_init(){
  ...
  TCA6416A_mode_set(&mode_buf);
  TCA6416A_gpio_service(&value_buf);
}
...
void gpio_init(){
  ...
  // xTaskCreate(tca6416a_int_task,"tca6416a_int_task",4096,NULL,10,NULL);//中断识别任务创建
  // gpio_install_isr_service(ESP_INTR_FLAG_LEVEL6);//安装服务时选择了第6优先级（高优先级）
  // gpio_isr_handler_add(TCA6416A_IO_INT,tca6416a_int_isr_handler,NULL);//为选定的GPIO添加ISR句柄
...
写：先对ext_io_value_data这个全局公共变量写入需要修改的值，再调用该函数控制IO电平反转
...
/* 扩展gpio输入输出模式服务
写：先对ext_io_mode_data这个全局公共变量写入需要修改的值，再调用该函数控制IO输入输出模式
