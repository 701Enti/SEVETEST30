...
//         同时成员变量名是上级程序识别操作引脚的关键，如果需要使用其上级程序而不仅仅是TCA6416A库函数，结构体成员 变量名 不应该修改，即使是对当前硬件的更新，否则应该全部修改
...
//必须确保定义时成员对应的IO次序不变，定义顺序必须为引脚顺序，比如 P00 P01 P02 P03 对应 bool l4,bool s4,bool l3,bool s3....以此类推,因为写入时按照成员地址的顺序到P0-P17，不能乱
